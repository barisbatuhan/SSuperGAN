...
class SSuperGlobalDCGANTrainer(BaseTrainer):
    def __init__(self,
                 model: SSuperGlobalDCGAN,
                 model_name: str,
                 criterion,
                 train_loader,
                 test_loader,
        ...
        if criterion["loss_type"] == "wgan":
            self.local_gan_loss = WGAN_GP(self.model, drift=self.criterion["alpha"], 
                                          use_gp=self.criterion["use_gp"], local=True)
            self.global_gan_loss = WGAN_GP(self.model, drift=self.criterion["alpha"], 
                                           use_gp=self.criterion["use_gp"], local=False)
        elif criterion["loss_type"] == "basic":
